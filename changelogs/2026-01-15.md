...
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│   Parallel Agents (Session-based)                                │
│   ├── ParallelAgentManager (src/core/async-agent.ts)            │
...
│   │                                                               │
│   ├── Tools (src/tools/async-agent.ts)                           │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│   Background Tasks (Shell commands)                               │
│   ├── BackgroundTaskManager (src/core/background.ts)             │
│   │   └── child_process.spawn() for command execution            │
│   │                                                               │
│   ├── Tools (src/tools/background.ts)                            │
│   │   ├── run_background   - Start command in background         │
│   │   ├── check_background - Check status and output             │
│   │   ├── list_background  - List all background tasks           │
│   │   └── kill_background  - Terminate running task              │
│   │                                                               │
└─────────────────────────────────────────────────────────────────┘
```
...
1. spawn_agent({ agent: "builder", prompt: "Implement feature X" })
   ...
   → task_001 and task_002 start in parallel sessions

2. run_background({ command: "npm run build" })
   → job_001 starts in background
...
4. [System notification appears]
   ...
   ✅ task_002: Review module Y

5. get_task_result({ taskId: "task_001" })
   → Full output from builder agent
...
| `src/tools/async-agent.ts` | spawn_agent, get_task_result, list_tasks, cancel_task |
...
Log output:
```
[parallel-agent] Acquired slot for builder: 1/3
[parallel-agent] Launched task_a1b2c3d4 in session sess_xyz
[parallel-agent] Completed task_a1b2c3d4
[parallel-agent] Notified parent: 2 tasks
[parallel-agent] Cleaned up task_a1b2c3d4 from memory
```
