...
- Auto-cleanup: 5 minutes after completion
    ...
    args: {
      ...
      prompt: tool.schema
        .string()
        .describe("Full prompt/instructions for the agent"),
...
- background=false: Wait for result (blocking)`;
      ...
      if (background === true) {
        try {
          ...
          console.log(
            `[parallel] 🚀 SPAWNED ${task.id} → ${agent}: ${description}`,
          );
...
✓ System will notify when ALL tasks complete. You can continue working!`;
      ...
      try {
        ...
        console.log(
          `[delegate] ✅ COMPLETED ${agent}: ${description} (${duration}s)`,
        );
 ...
 */
export const createGetTaskResultTool = (manager: ParallelAgentManager) =>
  tool({
...
Wait for the "All Complete" notification before checking.
    ...
    args: {
      taskId: tool.schema
        .string()
        .describe("Task ID from delegate_task (e.g., 'task_a1b2c3d4')"),
    },
    async execute(args) {
      const { taskId } = args;
 ...
 */
export const createListTasksTool = (manager: ParallelAgentManager) =>
  tool({
    ...
    args: {
      status: tool.schema
        .string()
        .optional()
        .describe("Filter: 'all', 'running', 'completed', 'error'"),
    },
    async execute(args) {
      ...
      switch (status) {
        ...
        case "error":
          tasks = manager
            .getAllTasks()
            .filter((t) => t.status === "error" || t.status === "timeout");
          break;
        default:
...
Use \`delegate_task({ ..., background: true })\` to spawn background tasks.`;
      ...
      const runningCount = manager.getRunningTasks().length;
      const completedCount = manager
        .getAllTasks()
        .filter((t) => t.status === "completed").length;
      const errorCount = manager
        .getAllTasks()
        .filter((t) => t.status === "error" || t.status === "timeout").length;
 ...
 */
export const createCancelTaskTool = (manager: ParallelAgentManager) =>
  tool({
...
Frees up concurrency slot for other tasks.
    ...
    args: {
      taskId: tool.schema
        .string()
        .describe("Task ID to cancel (e.g., 'task_a1b2c3d4')"),
    },
    async execute(args) {
      const { taskId } = args;
 ...
 */
export function createAsyncAgentTools(
  manager: ParallelAgentManager,
  client?: unknown,
