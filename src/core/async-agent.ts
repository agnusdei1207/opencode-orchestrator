...
export class ParallelAgentManager {
  ...
  static getInstance(
    ...
    if (!ParallelAgentManager._instance) {
      ...
      ParallelAgentManager._instance = new ParallelAgentManager(
        client,
        directory,
      );
    }
  ...
  async cancelTask(taskId: string): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `[parallel] 🗑️ Session ${task.sessionID.slice(0, 8)}... deleted`,
      );
    } catch {
      console.log(
        `[parallel] 🗑️ Session ${task.sessionID.slice(0, 8)}... already gone`,
      );
    }
    ...
    try {
      ...
      for (const task of runningTasks) {
        ...
        if (sessionStatus?.type === "idle") {
          ...
          console.log(
            `[parallel] ✅ COMPLETED ${task.id} → ${task.agent}: ${task.description} (${duration})`,
          );
    ...
    for (const [taskId, task] of this.tasks.entries()) {
      ...
      if (age > TASK_TTL_MS) {
        ...
        this.client.session
          ...
          .then(() => {
            console.log(
              `[parallel] 🗑️ CLEANED ${taskId} (timeout session deleted)`,
            );
          })
          .catch(() => {
            console.log(
              `[parallel] 🗑️ CLEANED ${taskId} (timeout session already gone)`,
            );
          });
  ...
  private async notifyParentIfAllComplete(
    parentSessionID: string,
  ): Promise<void> {
    const pending = this.pendingByParent.get(parentSessionID);
    ...
    const summary = completedTasks
      .map((t) => {
        ...
        const duration = this.formatDuration(t.startedAt, t.completedAt);
        return `${status} \`${t.id}\` (${duration}): ${t.description}`;
      })
      .join("\n");
