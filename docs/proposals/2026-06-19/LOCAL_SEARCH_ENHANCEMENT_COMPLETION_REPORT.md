...
| 4 | Low | The proposal's "구현 반영 업데이트" section listed writeback, tier moves, and tombstones as *deferred*, but the commits implemented them; it also named `memory-consolidation.ts` for lifecycle (actually `memory-lifecycle.ts`); the named test file did not exist. | Proposal section rewritten to match shipped reality; file reference corrected; decay constants documented as living in `memory-scoring.ts`. |
